    ...
    def fit_thresholds(self,
        ...
        if isinstance(alpha, list):
            if validation_data is None and cv is None:
                ...
                if use_hidden_feature:
                    batch_num = int(
                        ceil(val_inputs.shape[0] / float(batch_size)))
                    for current_batch in range(batch_num):
                        start = current_batch * batch_size
...
class MLC_GRU(Model):
    ...
    def fit_thresholds(self,
        ...
        if use_hidden_feature:
            # get outputs of conv layer
            vec_model = K.function([
                self.get_layer('title').input,
                self.get_layer('content').input
            ], [self.get_layer('H').output])

            # get outputs of conv layer
            batch_num = int(ceil(data['title'].shape[0] / float(batch_size)))
            for current_batch in range(batch_num):
                start = current_batch * batch_size
                step = batch_size if current_batch + 1 != batch_num else data['title'].shape[0] - current_batch * batch_size
                batch_inputs_title = data['title'][start:start + step]
                batch_inputs_content = data['content'][start:start + step]
                batch_outputs = vec_model(
                    [batch_inputs_title, batch_inputs_content])[0]
                t_inputs = batch_outputs if current_batch == 0 else np.concatenate(
                    (t_inputs, batch_outputs), axis=0)

        else:
            print('only use_hidden_feature support.')
        probs = self.predict(data, batch_size=batch_size)
        ...
        if isinstance(alpha, list):
            if validation_data is None and cv is None:
                ...
                if use_hidden_feature:
                    batch_num = int(
                        ceil(validation_data['title'].shape[0] / float(
                            batch_size)))
                    for current_batch in range(batch_num):
                        start = current_batch * batch_size
                        step = batch_size if current_batch + 1 != batch_num else validation_data['title'].shape[0] - current_batch * batch_size
                        batch_inputs_title = validation_data['title'][
                            start:start + step]
                        batch_inputs_content = validation_data['content'][
                            start:start + step]
                        batch_outputs = vec_model(
                            [batch_inputs_title, batch_inputs_content])[0]
                        val_t_inputs = batch_outputs if current_batch == 0 else np.concatenate(
